...
public class ElderManager : MonoBehaviour
{
    ...
    Pickup assignedTask = Pickup.None;
    [SerializeField]
    ...
    {
        speech.LoadText("what up homeslice");

        if (assignedTask == Pickup.None)
            GiveTask();
        else
    ...
    {
        if (given_item != assignedTask)
        {
            speech.LoadText("yo wtf is that shit");
            return;
        }

        FindObjectOfType<PlayerController>().heldItem = Pickup.None;

        completedTasks |= given_item;
        speech.LoadText("thanks homie i owe u");
        assignedTask = 0x0;

        if (completedTasks != Pickup.PICKUP_LENGTH) // if not all tasks are done
        {
            speech.LoadText("yo i need something else tho");
            GiveTask();
        }
        else //else if everything is done
            speech.LoadText("that all folks");
    }
    ...
    public void GiveTask()
    {
        ...
        while (assignedTask == Pickup.None)
        {
            ...
            if (!completedTasks.HasFlag(new_task))
            {
                ...
                speech.LoadText("go get me this " + assignedTask + " bro");
            }
        }
    }
}
